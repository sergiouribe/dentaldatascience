...
```{r}
prop.table(table(df$age_grp, df$cc_grp), 2)*100
```
This is better. 
...
df %>%
  ...
  spread(cc_grp, prop)
```
I still don't figure it how to make prop table with % for column with dplyr, but [found](https://stackoverflow.com/questions/40266062/dplyr-how-to-get-two-way-tables-with-marginal-proportions) a solution using the [janitor](https://cran.r-project.org/web/packages/janitor/index.html) package. 
```{r}
df %>% 
